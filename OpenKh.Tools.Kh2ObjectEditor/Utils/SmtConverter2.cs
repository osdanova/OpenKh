...
namespace OpenKh.Tools.Kh2ObjectEditor.Utils
{
    public class SmtConverter2
    {
        ...
        {
            if (kh2Model == null)
                kh2Model = MdlxService.Instance.ModelFile;
            if (kh2TextureFile == null)
                kh2TextureFile = MdlxService.Instance.TextureFile;
            if (kh2Animation == null)
                kh2Animation = MsetService.Instance.LoadedMotion;
        ...
        // Cross-checked against the actual PS2 decompilation (kn::MotionPrototype0::calc /
        // calcMatrix_ignoreScale in kn.c). Constraints in KH2 do not bend anything themselves: each one just
        // overwrites a single joint's global position/rotation/scale, copied from a source joint (or, for
        // Two-Points, the midpoint of two sources). The bend of a 2-bone limb (e.g. a hip/knee bending so a
        // Position-constrained ankle reaches its IK Helper target) is a *separate* mechanism, keyed off the
        // "ik" bits (Joint.Flags & 0x3) rather than off the Constraint or the "calculated" flag:
        //   - A joint with ik==1 is the ROOT of a single-joint aim chain: (root, effector=root+1). ik>=2 is the
        //     ROOT of a two-bone bend chain: (root, mid=root+1, effector=root+2) - unless Flags & 0x80
        //     ("extEffector") is set, in which case the effector id instead comes from the file's
        //     ExternalEffectors table, consumed by a cursor that only advances for ik!=0 joints, in Joints order.
        //   - The chain's target is wherever the effector's own active Position (or Two-Points) Constraint
        //     points. No such constraint -> the engine silently clears the ik bits for that root (plain FK).
        //   - The bend itself is analytic two-bone IK (law of cosines on the two bind bone lengths, elbow plane
        //     taken from the chain's current/un-bent geometry) - matching calcIKJoint's signature (self, target,
        //     len0, len1, sgn, mat0, mat1, prot0, prot1) - NOT iterative CCD.
        //   - Joint.Flags & 0x20 ("calculated") means the OPPOSITE of what it sounds like: it tells the engine
        //     to *skip* this joint's own FK/constraint evaluation because something else (the IK solver, or an
        //     external caller) already wrote its transform. It's an output marker, not a "bend me" input marker,
        //     and isn't used for chain discovery here.
        // Every joint's LOCAL pose (bind + InitialPose + F-Curve) is computed once, up front, for all bones and
        // IK Helpers alike, storing S/R/T separately so Expressions can read other joints' raw channel values.
        // Then, in the file's own Joints traversal order (parents before children), each joint's Expressions are
        // evaluated, followed by the bend/aim solve (if this joint is a chain root), followed by its ordinary
        // Constraints. Global transforms are resolved recursively through the (mutable) local matrices, so a
        // joint sees any correction applied earlier in Joints order by its ancestors.
        // Constraint types Position/Orientation/Scale/Direction/Up-Vector/Two-Points are applied; Path/Camera*/
        // Int*/Position-Limiter/Rotation-Limiter are recognized (Two-Points' LimiterId doubles as a second
        // source joint id, per the decomp) but Limiters themselves are not implemented - their exact clamp
        // formula wasn't recoverable from the decompilation.
        public static Dictionary<float, Matrix4x4[]> getMatricesForKeyFrames(ModelSkeletal model, AnimationBinary animation, HashSet<float> keyframeTimes)
        {
            ...
            // MotionHeader.SubType == IGNORE_SCALE selects calcMatrix_ignoreScale in the real engine, which never
            // reads or writes any scale channel at all (bind pose, InitialPose, F-Curve or Expression) - the
            // joint simply keeps whatever scale composition its parent chain already implies.
            bool ignoreScale = motionFile.MotionHeader.SubType == (int)Motion.MotionSubType.IGNORE_SCALE;
            ...
            // The "rotation" bit (0x8) marks a joint whose global rotation is externally supplied/preserved
            // rather than recomputed from its parent (per the decomp, the counterpart of the "trans" bit 0x4
            // which does the same for translation). For a static exporter with no prior-frame state, the only
            // case where that's observable is an IK chain's effector (e.g. an ankle) keeping its own baked
            // rotation while the freshly-solved knee/hip above it move - so it's masked off everywhere except
            // confirmed IK effectors (ikEffectorJoints, built below) to avoid discarding a real parent rotation
            // on ordinary FK-only bones that happen to also set this bit. See getGlobalTransform.
            ...
            // Chain discovery, straight off the "ik" bits (Flags & 0x3) of each joint - see the header comment
            // above for the root/mid/effector layout. bendChainByRoot covers ik>=2 (two-bone bend); aimStepByRoot
            // covers ik==1 (single-joint aim, e.g. the second+ link of a rope/ribbon strip where each link is
            // its own ik==1 joint rather than a multi-link "aim chain" construct - the engine has no such thing).
            Dictionary<int, (int mid, int tip, int source)> bendChainByRoot = new Dictionary<int, (int, int, int)>();
            Dictionary<int, (int tip, int source)> aimStepByRoot = new Dictionary<int, (int, int)>();
            HashSet<int> ikEffectorJoints = new HashSet<int>();

            int externalEffectorCursor = 0;
            foreach (Motion.Joint joint in motionFile.Joints)
            {
                int ik = joint.Flags & 0x3;
                if (ik == 0)
                    continue;

                int effector;
                if ((joint.Flags & 0x80) != 0)
                {
                    // extEffector: the effector id comes from the file's own table instead of being implied by
                    // id+1/id+2. The cursor only advances for ik!=0 joints (matching the engine's forward-only
                    // read), regardless of whether a usable target constraint ends up being found below.
                    effector = externalEffectorCursor < motionFile.ExternalEffectors.Count
                        ? motionFile.ExternalEffectors[externalEffectorCursor++].JointId
                        : -1;
                }
                else
                {
                    effector = joint.JointId + (ik == 1 ? 1 : 2);
                }

                if (effector < 0 || effector >= totalJointCount)
                    continue;

                if (!constraintsByTarget.TryGetValue(effector, out List<Motion.Constraint> effectorConstraints))
                    continue;

                Motion.Constraint targetConstraint = effectorConstraints.FirstOrDefault(c =>
                    (Motion.ConstraintType)c.Type == Motion.ConstraintType.POSITION ||
                    (Motion.ConstraintType)c.Type == Motion.ConstraintType.TWO_PNTS);
                if (targetConstraint == null)
                    continue; // no target: the engine clears the ik bits here too, joint stays plain FK

                ikEffectorJoints.Add(effector);
                if (ik == 1)
                    aimStepByRoot[joint.JointId] = (effector, targetConstraint.SourceJointId);
                else
                    bendChainByRoot[joint.JointId] = (joint.JointId + 1, effector, targetConstraint.SourceJointId);
            }

            // Mask flag 0x8 off everywhere except confirmed IK effectors now that ikEffectorJoints is known -
            // see the comment on jointFlags above.
            for (int i = 0; i < totalJointCount; i++)
                if ((jointFlags[i] & 0x8) != 0 && !ikEffectorJoints.Contains(i))
                    jointFlags[i] &= ~0x8;
            ...
            foreach (float keyTime in keyframeTimes)
            {
                ...
                for (int i = 0; i < totalJointCount; i++)
                {
                    ...
                    scales[i] = ignoreScale ? Vector3.One : bindPose.Scale;
                    ...
                    if (curvesByJoint.TryGetValue(i, out List<Motion.FCurve> curves))
                        ...
                        {
                            int channel = curve.Channel & 0xF;
                            if (!ignoreScale || channel > 2)
                                setChannel(ref scales[i], ref rotations[i], ref translations[i], channel, evaluateFCurve(motionFile, curve, keyTime));
                        }
                ...
                // chains are resolved exactly when the loop reaches their root joint, rather than in a separate
                // later pass, specifically so a joint further down the same lineage that depends on this one
                // (whether via a regular constraint's back-solve or another bend/aim step) always sees its final
                // rotation - see the comment on bendChainByRoot/aimStepByRoot above.
                foreach (Motion.Joint joint in motionFile.Joints)
                {
                    ...
                    if (expressionsByTarget.TryGetValue(jointId, out List<Motion.Expression> expressions))
                    {
                        ...
                        {
                            if (ignoreScale && expression.TargetChannel <= 2)
                                continue;
                    ...
                    if (bendChainByRoot.TryGetValue(jointId, out var bendChain))
                    {
                        Vector3 targetPosition = getGlobalTransform(localMatrices, parentIds, jointFlags, bendChain.source).Translation;
                        bendTwoBoneChain(localMatrices, parentIds, jointFlags, jointId, bendChain.mid, bendChain.tip, targetPosition);
                    }

                    if (aimStepByRoot.TryGetValue(jointId, out var aimStep))
                    {
                        Vector3 targetPosition = getGlobalTransform(localMatrices, parentIds, jointFlags, aimStep.source).Translation;
                        aimChildTowardTarget(localMatrices, parentIds, jointFlags, jointId, aimStep.tip, targetPosition);
                    }
                    ...
                    if (constraintsByTarget.TryGetValue(jointId, out List<Motion.Constraint> constraints))
                        ...
                        {
                            // An IK chain effector's Position/Two-Points constraint is already satisfied by the
                            // bend/aim itself (that's the whole point of it being an effector) - applying it again
                            // would be redundant, not wrong, but skip it for clarity. Any OTHER constraint type on
                            // the same joint (Orientation, Scale, ...) is independent of the position solve and
                            // must still run: a joint can legitimately carry both a Position AND an Orientation
                            // constraint from the same source (a full rigid lock, not a free "stretch to position"
                            // rope link), and the bend/aim machinery only ever resolves the position side of that.
                            Motion.ConstraintType constraintType = (Motion.ConstraintType)constraint.Type;
                            if (ikEffectorJoints.Contains(jointId) &&
                                (constraintType == Motion.ConstraintType.POSITION || constraintType == Motion.ConstraintType.TWO_PNTS))
                                continue;
                            applyConstraint(motionFile, localMatrices, parentIds, jointFlags, constraint, keyTime);
                        }

                    // calcMatrix2Rot (0x40): the engine writes the joint's just-finished local matrix back into
                    // its own Euler rotation channel, so any OTHER joint's Expression evaluated later this same
                    // frame that reads this joint's FCURVE_ROTX/Y/Z sees the post-IK/post-constraint value instead
                    // of the pre-solve F-Curve value.
                    if ((joint.Flags & 0x40) != 0)
                        rotations[jointId] = decomposeLocalRotationXYZ(localMatrices[jointId]);
                }
        ...
        // normally). This is only honored for confirmed IK effectors (see ikEffectorJoints/jointFlags masking in
        // getMatricesForKeyFrames) - e.g. an ankle: once the knee/hip above it are bent to reach the IK target,
        // the effector's baked rotation was never meant to inherit that freshly-solved parent orientation, only
        // to sit at the right world position.
        ...
        private static bool isConstraintActive(Motion.InterpolatedMotion motionFile, Motion.Constraint constraint, float frameTime)
        {
            ...
            // Each activation key records a state *transition* at its time, so before the very first key the
            // constraint was in the opposite state - the decomp inverts the first key's Active value for exactly
            // this window (kn.c calcConstraintActivation), rather than defaulting to "active" the way a naive
            // "most recent key at/ before frameTime" scan would.
            Motion.ConstraintActivation firstActivation = motionFile.ConstraintActivations[constraint.ActivationStartId];
            if (frameTime < firstActivation.Time)
                return firstActivation.Active == 0;
        ...
        private static void applyConstraint(Motion.InterpolatedMotion motionFile, Matrix4x4[] localMatrices, int[] parentIds, int[] jointFlags, Motion.Constraint constraint, float keyTime)
        {
            ...
            switch ((Motion.ConstraintType)constraint.Type)
            {
                ...
                case Motion.ConstraintType.DIRECTION:
                {
                    ...
                    // target at the source, changing only orientation.
                    Vector3 sourceWorldPos = getGlobalTransform(localMatrices, parentIds, jointFlags, source).Translation;
                    aimLocalXAt(localMatrices, parentIds, jointFlags, target, sourceWorldPos);
                    break;
                }
                case Motion.ConstraintType.TWO_PNTS:
                {
                    // Confirmed in the decomp (getCnsPos: two active_ids -> the average of both global
                    // positions) to position the joint at the MIDPOINT of two sources, in addition to aiming its
                    // local X axis at the second one - LimiterId doubles as that second source joint id for this
                    // constraint type (it's only a Limiter table index for POS_LIM/ROT_LIM).
                    Vector3 pos1 = getGlobalTransform(localMatrices, parentIds, jointFlags, source).Translation;
                    Vector3 pos2 = getGlobalTransform(localMatrices, parentIds, jointFlags, constraint.LimiterId).Translation;
                    Vector3 midpoint = (pos1 + pos2) * 0.5f;

                    Matrix4x4 current = getGlobalTransform(localMatrices, parentIds, jointFlags, target);
                    current.Translation = midpoint;
                    setGlobalTransform(localMatrices, parentIds, jointFlags, target, current);

                    aimLocalXAt(localMatrices, parentIds, jointFlags, target, pos2);
                    break;
                }
                ...
                default:
                    break; // Path/Camera*/Int*/Position-Limiter/Rotation-Limiter are not implemented
            }
        }

        // Aims the target joint's local +X axis at 'aimWorldPos', changing only its orientation (world position
        // untouched) - shared by DIRECTION and TWO_PNTS. A separate Up-Vector constraint (a distinct Constraint
        // entry, type UP_VECTOR) would normally control roll/twist around that axis; when none targets this
        // joint, empirically (checked against the PS2 emulator) the up-vector Softimage falls back to is the
        // target's own parent's current Y axis, not world-up or the target's own pre-constraint roll - i.e. a
        // classic look-at using the parent's orientation as the up reference.
        private static void aimLocalXAt(Matrix4x4[] localMatrices, int[] parentIds, int[] jointFlags, int target, Vector3 aimWorldPos)
        {
            ...
            Vector3 aimDirWorld = aimWorldPos - targetWorldPos;
            ...
            // The parent's Y axis is only a well-conditioned up-vector reference when it isn't nearly
            // parallel/antiparallel to the aim direction - verified against the PS2 emulator on two separate
            // rigs where the aim happens to run almost straight along the parent's Y axis: the cross product's
            // magnitude isn't literally zero there (so the old 1e-10 threshold never caught it), but its
            // *direction* is dominated by floating-point noise, producing a roll up to 180 degrees off. Parent's
            // Z axis, being orthogonal to Y, is always well-conditioned exactly when Y isn't (an aim direction
            // can't be near-parallel to two orthogonal axes at once), so fall back to it whenever the Y-axis
            // configuration is anywhere close to degenerate, not just exactly degenerate.
            Vector3 zAxis;
        ...
        // Bends 'root' and 'mid' so that 'tip' reaches 'targetPosition', using analytic two-bone IK (law of
        // cosines on the two current bone lengths) - matching kn::Motion::calcIKJoint's signature (self, target,
        // len0, len1, sgn, mat0, mat1, prot0, prot1: two bone lengths, an elbow-side sign and two output
        // matrices, the classic two-bone-IK parameter set), not iterative CCD. Bone lengths are read live off the
        // chain's own current (pre-bend, FK) geometry rather than the bind pose, so this stays correct under
        // per-frame scale/stretch.
        //
        // The elbow/bend plane is taken from the chain's own current geometry (the plane through root, mid and
        // the target) rather than a baked "preferred rotation": projecting the current root->mid direction
        // perpendicular to root->target gives a stable, deterministic pole vector that keeps the limb bending on
        // whichever side it's already naturally posed toward, with no ambiguity to resolve and no iteration.
        private static void bendTwoBoneChain(Matrix4x4[] localMatrices, int[] parentIds, int[] jointFlags, int root, int mid, int tip, Vector3 targetPosition)
        {
            ...
            Vector3 midPos = getGlobalTransform(localMatrices, parentIds, jointFlags, mid).Translation;
            Vector3 tipPos = getGlobalTransform(localMatrices, parentIds, jointFlags, tip).Translation;

            float len0 = Vector3.Distance(rootPos, midPos);
            float len1 = Vector3.Distance(midPos, tipPos);
            if (len0 < 1e-6f || len1 < 1e-6f)
                return; // degenerate (zero-length) bone - nothing meaningful to bend

            Vector3 toTarget = targetPosition - rootPos;
            float targetDist = toTarget.Length();
            if (targetDist < 1e-6f)
                return; // target sits on the root joint

            // Clamp the reachable distance to just inside [|len0-len1|, len0+len1] so the law-of-cosines acos
            // below never receives an out-of-[-1,1] ratio (an unreachable target, e.g. a stretched-out limb
            // reaching past its max extension) - this is what makes the limb stop at full extension instead of
            // producing NaNs.
            float maxReach = len0 + len1;
            float minReach = MathF.Abs(len0 - len1);
            const float epsilon = 1e-4f;
            float d = maxReach - minReach > 2f * epsilon
                ? Math.Clamp(targetDist, minReach + epsilon, maxReach - epsilon)
                : Math.Clamp(targetDist, 0f, maxReach);

            Vector3 axis = toTarget / targetDist;
            Vector3 rootToMid = midPos - rootPos;
            Vector3 pole = rootToMid - axis * Vector3.Dot(rootToMid, axis);
            if (pole.LengthSquared() < 1e-10f)
                // Chain is already colinear with the target (no bend direction implied by current geometry) -
                // fall back to any axis perpendicular to the aim direction.
                pole = Vector3.Cross(axis, Math.Abs(axis.Y) < 0.99f ? Vector3.UnitY : Vector3.UnitX);
            pole = Vector3.Normalize(pole);

            float cosRootAngle = Math.Clamp((len0 * len0 + d * d - len1 * len1) / (2f * len0 * d), -1f, 1f);
            float rootAngle = MathF.Acos(cosRootAngle);
            Vector3 newMidDir = axis * MathF.Cos(rootAngle) + pole * MathF.Sin(rootAngle);
            Vector3 newMidPos = rootPos + newMidDir * len0;

            // Two direct swings - root so mid lands on its solved position, then mid so tip lands on the actual
            // target - are both necessary and sufficient for a two-bone chain; no iteration needed.
            aimChildTowardTarget(localMatrices, parentIds, jointFlags, root, mid, newMidPos);
            aimChildTowardTarget(localMatrices, parentIds, jointFlags, mid, tip, targetPosition);
        }
        ...
        // The inverse of buildLocalMatrix's rotation half: recovers the X/Y/Z Euler angles that, fed back through
        // CreateRotationX*CreateRotationY*CreateRotationZ, reproduce the same rotation. Used for flag 0x40
        // (calcMatrix2Rot) so a joint's post-IK/post-constraint rotation is visible to other joints' Expressions
        // evaluated later the same frame, instead of the pre-solve F-Curve value.
        private static Vector3 decomposeLocalRotationXYZ(Matrix4x4 local)
        {
            Matrix4x4.Decompose(local, out _, out Quaternion rotation, out _);
            Matrix4x4 r = Matrix4x4.CreateFromQuaternion(Quaternion.Normalize(rotation));

            float y = MathF.Asin(Math.Clamp(-r.M13, -1f, 1f));

            float x, z;
            if (Math.Abs(r.M13) < 0.9999f)
            {
                x = MathF.Atan2(r.M23, r.M33);
                z = MathF.Atan2(r.M12, r.M11);
            }
            else
            {
                // Gimbal lock (Y at +-90 degrees): X and Z rotate around the same effective axis - fold
                // everything into X and leave Z at 0, same convention as the engine's own FMatrix::toRot.
                float sy = MathF.Sign(-r.M13);
                x = MathF.Atan2(r.M21 * sy, r.M22);
                z = 0f;
            }
            ...
            Matrix4x4[] localMatrices, int[] parentIds, int[] jointFlags, float keyTime, int index)
        {
            ...
            switch (nodeType)
            {
                case Motion.ExpressionType.FUNC_SIN:
                    return MathF.Sin(degToRad(Evaluate(node.CAR)));
                case Motion.ExpressionType.FUNC_COS:
                    return MathF.Cos(degToRad(Evaluate(node.CAR)));
                case Motion.ExpressionType.FUNC_TAN:
                    return MathF.Tan(degToRad(Evaluate(node.CAR)));
                ...
                case Motion.ExpressionType.FUNC_ATAN:
                    return radToDeg(MathF.Atan(Evaluate(node.CAR)));
                case Motion.ExpressionType.FUNC_LOG:
                    return MathF.Log(Evaluate(node.CAR));
                case Motion.ExpressionType.FUNC_EXP:
                    return MathF.Exp(Evaluate(node.CAR));
                case Motion.ExpressionType.FUNC_ABS:
                    return MathF.Abs(Evaluate(node.CAR));
                case Motion.ExpressionType.FUNC_POW:
                    return MathF.Pow(Evaluate(node.CAR), Evaluate(node.CDR));
                case Motion.ExpressionType.FUNC_SQRT:
                    return MathF.Sqrt(MathF.Abs(Evaluate(node.CAR)));
                case Motion.ExpressionType.FUNC_MIN:
                    return Math.Min(Evaluate(node.CAR), Evaluate(node.CDR));
                case Motion.ExpressionType.FUNC_MAX:
                    return Math.Max(Evaluate(node.CAR), Evaluate(node.CDR));
                ...
                case Motion.ExpressionType.FUNC_FMOD:
                    return Evaluate(node.CAR) % Evaluate(node.CDR);
                case Motion.ExpressionType.OP_PLUS:
                    return Evaluate(node.CAR) + Evaluate(node.CDR);
                case Motion.ExpressionType.OP_MINUS:
                    return Evaluate(node.CAR) - Evaluate(node.CDR);
                case Motion.ExpressionType.OP_MUL:
                    return Evaluate(node.CAR) * Evaluate(node.CDR);
                case Motion.ExpressionType.OP_DIV:
                    return Evaluate(node.CAR) / Evaluate(node.CDR);
                case Motion.ExpressionType.OP_MOD:
                    return MathF.Round(Evaluate(node.CAR)) % MathF.Round(Evaluate(node.CDR));
                case Motion.ExpressionType.OP_EQ:
                    return Evaluate(node.CAR) == Evaluate(node.CDR) ? 1f : 0f;
                case Motion.ExpressionType.OP_GT:
                    return Evaluate(node.CAR) > Evaluate(node.CDR) ? 1f : 0f;
                case Motion.ExpressionType.OP_GE:
                    return Evaluate(node.CAR) >= Evaluate(node.CDR) ? 1f : 0f;
                case Motion.ExpressionType.OP_LT:
                    return Evaluate(node.CAR) < Evaluate(node.CDR) ? 1f : 0f;
                case Motion.ExpressionType.OP_LE:
                    return Evaluate(node.CAR) <= Evaluate(node.CDR) ? 1f : 0f;
                case Motion.ExpressionType.OP_AND:
                    return Evaluate(node.CAR) >= 1f && Evaluate(node.CDR) >= 1f ? 1f : 0f;
                case Motion.ExpressionType.OP_OR:
                    return Evaluate(node.CAR) >= 1f || Evaluate(node.CDR) >= 1f ? 1f : 0f;
                case Motion.ExpressionType.VARIABLE_FC:
                    return keyTime;
                case Motion.ExpressionType.CONSTANT_NUM:
                    return node.Value;
                case Motion.ExpressionType.FCURVE_ETRNX:
                    return getExpressionChannel(translations, node).X;
                case Motion.ExpressionType.FCURVE_ETRNY:
                    return getExpressionChannel(translations, node).Y;
                case Motion.ExpressionType.FCURVE_ETRNZ:
                    return getExpressionChannel(translations, node).Z;
                case Motion.ExpressionType.FCURVE_ROTX:
                    return radToDeg(getExpressionChannel(rotations, node).X);
                case Motion.ExpressionType.FCURVE_ROTY:
                    return radToDeg(getExpressionChannel(rotations, node).Y);
                case Motion.ExpressionType.FCURVE_ROTZ:
                    return radToDeg(getExpressionChannel(rotations, node).Z);
                case Motion.ExpressionType.FCURVE_SCALX:
                    return getExpressionChannel(scales, node).X;
                case Motion.ExpressionType.FCURVE_SCALY:
                    return getExpressionChannel(scales, node).Y;
                case Motion.ExpressionType.FCURVE_SCALZ:
                    return getExpressionChannel(scales, node).Z;
                case Motion.ExpressionType.ELEMENT_NAME:
                    return 0f; // Documented as a no-op
                default:
                    return 0f; // LIST is only ever consumed by FUNC_AV/FUNC_COND, never evaluated directly
            }
        }
